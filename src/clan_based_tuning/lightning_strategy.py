...
class ClanDDPStrategy(DDPStrategy):
    ...
    The strategy supplies only the topology facts Lightning cannot infer from separate
    Tune trials and disables DDP's per-forward buffer broadcast so member-local updates
    are not erased. Backend selection remains entirely with Lightning/PyTorch unless the
    user explicitly passes the ordinary ``process_group_backend`` argument.

    CBT round checkpointing temporarily marks one selected rank as the writer. Every rank
    still enters Lightning's ordinary checkpoint construction and barrier, but only that
    selected rank delegates the checkpoint to ``CheckpointIO``. Ordinary checkpoint calls
    outside that scoped operation retain Lightning's normal behavior.
    """
    ...
    def begin_round_checkpoint(self, source_rank: int) -> None:
        """Scope the next Lightning checkpoint write to one selected Clan member."""
    ...
    def save_checkpoint(
        ...
        """Persist only the selected round source while all ranks retain the barrier."""
