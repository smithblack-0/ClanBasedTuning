"""Ray collective runtime for one Clan population boundary."""

from collections.abc import Sequence


class RayPopulationRuntime:
    """All-gather one fitness value per stable Clan member through Ray GLOO.

    Collective rank is transport state, not member identity. The runtime records the
    explicit mapping and returns fitness keyed by stable member ID. One collective group
    is initialized and destroyed around each resolution call.
    """
