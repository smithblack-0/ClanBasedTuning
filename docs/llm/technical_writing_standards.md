...
These standards define how to judge substantial ClanBasedTuning writing:
...
They are reference criteria, not an execution sequence and not a source of
project authority. Use the
[technical-writing workflow](technical_writing_workflow.md) to conduct the work;
consult the relevant standards when defining, drafting, reviewing, or correcting
an artifact.
...
Every substantial artifact must have a clear communication contract:

1. **Transmission boundary:** Is it standalone or one part of a larger
   documentation path?
2. **Artifact role:** What unique job does it perform? What may it assume, what
   must it establish, and where should it hand the reader next?
...
If these answers are unclear, prose is premature. The artifact role or supporting
project model must be resolved first.
...
- undocumented public behavior and stale or competing explanations; and
...
- Does it cover material consequences rather than merely describe nearby
  technology?
...
- Are unsupported aspirations expressed as intent, hypothesis, or candidate
  scope rather than current fact?
...
## Technical-substance standard

Include enough material for the intended reader to answer the relevant subset of
these questions:
...
order, or implementation structure.
...
The workflow runs these as independent passes. A failure in one pass should be
classified and routed to the stage that owns the defect rather than patched
locally by default.
